...
class ChapterController {
    constructor() {
        ...
        this.uploadChapters = (req, res) => __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                for (const chapterData of chaptersData) {
                    try {
                        ...
                        validChapters.push(chapterData);
                    }
                    catch (error) {
                        failedChapters.push({
                            chapter: chapterData,
                ...
                if (validChapters.length > 0) {
                    try {
                        const savedChapters = yield chapter_model_1.default.insertMany(validChapters, { ordered: false });
                        successfulUploads = savedChapters.length;
                    }
                    catch (error) {
                        for (const chapterData of validChapters) {
                            try {
                                yield new chapter_model_1.default(chapterData).save();
                                successfulUploads++;
                            }
                            catch (saveError) {
                                failedChapters.push({
        ...
        this.getChapters = (req, res) => __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                // Check Redis cache
                const cachedChapters = yield redis.get(cacheKey);
                ...
                const chapters = yield chapter_model_1.default.find(query).skip(skip).limit(limit);
                const totalChapters = yield chapter_model_1.default.countDocuments(query);
                // Cache the result in Redis
                yield redis.set(cacheKey, JSON.stringify({ chapters, totalChapters }), { ex: 3600 });
                res.status(200).json({
                    success: true,
                    data: chapters,
                    pagination: {
        ...
        this.getChapterById = (req, res) => __awaiter(this, void 0, void 0, function* () {
            try {
